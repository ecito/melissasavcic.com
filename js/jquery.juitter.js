...
(function($) {
	...
	$.Juitter = {
					...
					$.each(json.results,function(i,item) {
						if(e==1 || (i<numMSG && item.id>ultID)){
							...
							if (item.text != "undefined") {
								...
								if(fromID=="image") mHTML="<a href='http://www.twitter.com/"+item.from_user+"' target=_blank><img src='"+item.profile_image_url+"' alt='"+item.from_user+"' target=_blank class='juitterAvatar' /></a><br><br> "+$.Juitter.textFormat(item.text)+" <br><br><a href='" + link + "' class='JRM' target=_blank>"+readMore+"</a>";
		...
		textFormat: function(texto){
			...
			texto = texto.replace(exp,"<a href='$1' target=_blank class='extLink'>$1</a>"); 
